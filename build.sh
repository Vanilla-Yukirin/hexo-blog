...
# Hexo build: clean + generate (optional draft/encrypt)
...
echo "[REPRO] npx --yes hexo clean && npx --yes hexo g${DRAFT_SUFFIX} --config \"${CONFIG_ARG}\""

echo "[INFO] Running hexo clean"
npx --yes hexo clean

echo "[INFO] Running hexo generate"
if [[ "$USE_DRAFT" == true ]]; then
  npx --yes hexo g --draft --config "$CONFIG_ARG"
else
  npx --yes hexo g --config "$CONFIG_ARG"
fi
